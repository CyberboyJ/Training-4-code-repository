...
class OrderController extends Controller {
	...
	async creartOrder() {
		...
		const rule = {  //参数检验
			...
			orderDetails: { type: 'array', required: true, message: "商品详细信息" },//1.7
		};
	...
	async QueryOrderById() {
		console.log(this.ctx.params.gid);
		// {
		// 	"telId": , 
		// 	"addressId": , 
		// 	"orderTotal": ,
		...
		const validateErrors = parameter.validate(rule, { orderId: this.ctx.params.gid });//添加了orderId
		console.log(validateErrors)

		if (validateErrors) {
			this.ctx.body = validateErrors
		} else {
			let rs = await this.ctx.service.order.SelectOrderByID(this.ctx.params.gid);
			this.ctx.body = rs;
		}
	}
}
